...
const TimelineItem = ({ date, title, company, body, tools, end }: Props) => {
    ...
    return <li>
        ...
        <div className={`${!end === true ? "timeline-start" : "timeline-end"} md:text-end mb-10 min-w-[368px] pr-4 sm:pr-0 ${end === true ? "md:pr-4" : "md:pl-4"}`}>
            <time className="font-mono italic">{date}</time>
            <div className="text-lg lg:text-xl font-black">{title}</div>
            <div className="flex justify-between">
                <small className="mr-10">Tools:</small> <strong className="text-info ">{company}</strong>
            </div>
